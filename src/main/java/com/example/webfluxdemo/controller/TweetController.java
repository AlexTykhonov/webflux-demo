 ...
 */
// с помощтю аннотации РестКонтроллер все методы этого класса будут отправлять\получать http  запросы
@RestController
public class TweetController {

    // аннотация @Autowired позоваляет найти объект в спринг контейнере и инициализировать поле.
    @Autowired
    private TweetRepository tweetRepository; // Спринг реализует интерфейс и сделает все абстрактные методы рабочими

    // эта аннотация способна обработать http запрос через выполнение метода getAllTweets, ничего не принимает. Возвращает поток твитов.
    @GetMapping("/tweets")
